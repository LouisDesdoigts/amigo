...
def calc_lrs(model, exposures, fishers, params=None, fmax=1e64):
    ...
    grad_model, _ = eqx.partition(model, bool_model)
    ...
    # print(is_large)
    # large_grad_model, _ = eqx.partition(model, is_large)
    ...
    for exp in exposures:
        ...
        for param in params:
            ...
            # path = exp.get_key(param)
            # params_dict[path] += fishers[key]

            # # Check if the parameter is in the fisher
            # if param not in fishers[exp.key].keys():
            #     continue
            # fisher_model = fisher_model.add(param_path, fishers[exp.key])
            ...
            fisher_model = fisher_model.add(param_path, -fishers[f"{exp.key}.{param}"])
...
def optimise(
    ...
    # Paste in ones to parameters with no lr_model
    for param in opt_params:
        if lr_model.get(param) is None:
            lr_model = lr_model.set(param, np.array(1.0))

    # for param in opt_params:
    #     print(param, lr_model.get(param))
    ...
    for idx in looper:
        ...
        for i in batch_inds:
            ...
            # TEMPORARY HACK - assumes batch size == 1
            if batch_size != 1 and not warned:
                print("WARNING: losses only valid for batch size == 1")
                warned = True
            ...
            # for exp in batches[i]:
            #     if exp.observation == "FLAT":
            # # if batches[i][0].observation == "FLAT":
            #         _loss = 0
